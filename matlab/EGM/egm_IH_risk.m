...
function [pfun_cons, pfun_sav] = egm_IH_risk(par, tol, maxiter)
% EGM_IH_RISK   Solve the infinite-horizon HH problem with risky labour using EGM.
%
%   [PFUN_CONS, PFUN_SAV] = EGM_IH_RISK(PAR,TOL,MAXITER) returns the consumption and 
...
% Author: Richard Foltyn
    ...
    for iter = 1:maxiter
        ...
        for iy = 1:N_y
            ...
            %   mu = E[u'(C(a',y')) | y]
            mu = pfun_cons.^(-par.gamma) * par.tm_y(iy,:)';
            ...
            assets_sav = (cons_sav + par.grid_a - par.grid_y(iy)) / (1.0 + par.r);

            % The arrays (ASSETS_SAV, CONS_SAV) represent the updated
            % consumption policy function, defined on the endogenous asset
            % grid.
